...
RSpec.describe "AuthRequests", :type => :request do
  describe "POST /auth/sign_up" do
    ...
    context "valid request params" do
      before :each do
        post auth_sign_up_path, auth: valid_data
      end
    ...
    context "invalid request parameters" do
      before :each do
        ...
        post auth_sign_up_path, auth: invalid_data
      end
  ...
  describe "POST /auth/sign_in" do
    ...
    context "valid request params" do
      before :each do
        post auth_sign_in_path, auth: @valid_data
      end
    ...
    context "invalid password" do
      before :each do
        ...
        post auth_sign_in_path, auth: invalid_data
      end
  ...
  describe "GET /auth/token_status" do
    it "returns true if token is valid" do
      get auth_token_status_path, auth: {token: valid_token}
      expect(response).to have_http_status :success
    end

    it "returns false if token is invalid" do
      get auth_token_status_path, auth: {token: invalid_token}
      expect(response).to have_http_status :unauthorized
    end
  end
end
